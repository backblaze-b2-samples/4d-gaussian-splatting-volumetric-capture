...
#
# B2_PUBLIC_URL_BASE is deliberately NOT here: it only builds public object
# URLs for a public bucket and the app runs without it, so requiring it would
# needlessly block startup.
...
API_TITLE = "4D Gaussian Splatting Volumetric Capture API"
API_DESCRIPTION = (
    "Local API for 4D Gaussian Splatting Volumetric Capture: a capture-to-B2 "
    "pipeline that turns synchronized multi-camera video into a 4DGaussians "
    "dataset and trained volumetric splat, with every input and derived artifact "
    "versioned in Backblaze B2. This contract documents the template's local "
